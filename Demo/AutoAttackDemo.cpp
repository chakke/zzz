...
AutoAttackDemo::AutoAttackDemo() : AbstractDemo() {}
AutoAttackDemo::~AutoAttackDemo() {}

void AutoAttackDemo::onRecieveCommand(int type, std::shared_ptr<connection_object> data) {
...
void AutoAttackDemo::onGlobalTap(std::shared_ptr<connection_object> data)
{
	if (game::config->isBlockInput()) return;

}

void AutoAttackDemo::loadEntities()
{
	/*create background*/
	{
	...
	ECSParser::getInstance()->parse(mWorld, "xml/demo/auto-attack-demo-entities.xml");
}

void AutoAttackDemo::loadSystemAndConfig()
{
